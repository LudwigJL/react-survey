...
export default function Survey() {
  ...
  function handleChange(event) {
    let timeSpendArray = surveyData.timeSpent;
    ...
    if (inputType === "checkbox" && inputName === "spend-time") {
      if (event.target.checked) {
        timeSpendArray.push(inputValue);
      } else {
        timeSpendArray.splice(timeSpendArray.indexOf(inputValue), 1);
      }
      setSurveyData({ ...surveyData, timeSpent: timeSpendArray });
    }

    if (inputType === "radio" && inputName === "color") {
      setSurveyData({ ...surveyData, colour: inputValue });
    }
